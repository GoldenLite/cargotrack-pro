...
def compute_ed_status(hawb) -> str:
    ...
    # Отозванная накладная (выпуск снят вручную: возврат/реимпорт/ошибочный
    # выпуск) — ЭД-статус не показываем: старые сообщения (запрос док-тов,
    # проверка, отказы) относятся к отменённому оформлению и неактуальны.
    if getattr(hawb, 'customs_status', '') == 'WITHDRAWN':
        return ''
