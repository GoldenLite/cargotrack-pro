...
def apply_to_cargo(cargo: Cargo, parsed: dict, *, writeback: bool = True,
    ...
    # ── GUARD: частичное прибытие ──────────────────────────────────────────
    # Если склад принял МЕНЬШЕ мест, чем заявлено по авианакладной, то ДО1
    # оформлен только на прилетевшую часть. Какие именно накладные приехали —
    # из этих данных НЕ определить (провайдер даёт агрегат по партии, не
    # пер-накладную). Разлив ДО1/лицензии/даты прибытия на ВСЕ HAWB партии в
    # такой ситуации = ложное «всё прибыло»: декларант видит зелёную партию и
    # подаёт то, чего нет на складе.
    # Поэтому НЕ применяем ничего и ждём полного прибытия (или ручного
    # разбора). Кейс 784-84705375 (20.07.2026): 43 заявлено / 26 принято.
    if parsed.get('is_partial'):
        logger.warning(
            'external_warehouse: пропуск %s — частичное прибытие '
            '(заявлено %s мест, принято %s). СВХ/ДО1 не применяем.',
            cargo.awb_number, parsed.get('declared_pieces'),
            parsed.get('arrived_pieces'))
        return False
