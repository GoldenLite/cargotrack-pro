...
def _find_row_for_kind(hawb: HouseWaybill,
                       source_kind: str = 'general'
                       ) -> Optional[tuple[SheetSource, int]]:
    """Ищет в каком SheetSource заданного kind лежит строка с этим HAWB.

    source_kind='general' — обычная таблица (юзер вбивает HAWB вручную, мы
    дозаполняем). source_kind='export' — вкладка «Экспортная статистика»
    (мы сами добавляем строки через _ensure_export_row при появлении
    экспортной HAWB в outbox).
    """
    if not hawb.hawb_number:
        return None
    row = (
        ImportedSheetRow.objects
        .filter(source__kind=source_kind,
...
# Совместимость: тонкая обёртка, чтобы не менять все вызовы сразу.
def _find_general_row(hawb: HouseWaybill) -> Optional[tuple[SheetSource, int]]:
    return _find_row_for_kind(hawb, 'general')


def _get_export_source() -> Optional[SheetSource]:
    """Активный SheetSource с kind='export'. None если не настроен."""
    return SheetSource.objects.filter(kind='export', is_active=True).first()


def _ensure_export_headers(ws, header_row: int) -> dict[str, int]:
    """Проставляет все export-колонки в шапке вкладки.

    Если шапка пустая — записывает все EXPORT_HEADERS_ORDER в строку
    header_row по порядку A..I. Если шапка уже частично заполнена — для
    каждого недостающего заголовка добавляет в первую свободную справа
    (через _ensure_named_column).

    Возвращает {header_name: col_idx} — для дальнейших batch_update.
    """
    out: dict[str, int] = {}
    existing = ws.row_values(header_row) or []
    # Если шапки нет — записываем сразу всем блоком (один update).
    if not any((v or '').strip() for v in existing):
        ws.update(
            f'A{header_row}',
            [EXPORT_HEADERS_ORDER],
        ...
        for i, h in enumerate(EXPORT_HEADERS_ORDER, start=1):
            out[h] = i
            _col_index_cache[(ws_key, h)] = i
        logger.info('Export headers written to %s: %s',
                    ws.title, EXPORT_HEADERS_ORDER)
        return out
    # Шапка уже есть. Для каждого ожидаемого заголовка — найти/добавить.
    for h in EXPORT_HEADERS_ORDER:
        out[h] = _ensure_named_column(ws, header_row, h)
    return out
...
def _ensure_export_row(hawb: HouseWaybill) -> Optional[tuple[SheetSource, int]]:
    ...
    1. Находит активный SheetSource(kind='export').
    2. Открывает worksheet, гарантирует наличие шапки EXPORT_HEADERS_ORDER.
    3. Если в ImportedSheetRow(kind=export) уже есть запись для этого
       HAWB — возвращает её row_index.
    4. Иначе append_row с минимальным набором (HAWB + transport_doc), создаёт
...
def _do_append_export_row(hawb: HouseWaybill,
    ...
    try:
        ...
        logger.warning('export-row: open failed: %s', e)
        return None

    try:
        headers = _retry_api(_ensure_export_headers, ws, src.header_row,
                             label='export ensure_headers')
    except gspread.exceptions.APIError as e:
        logger.exception('export-row: header ensure failed: %s', e)
        return None

    # Проверим, нет ли уже строки с этим HAWB в Sheets (на случай ручной
    # вставки или если ImportedSheetRow рассинхронизирован).
    hawb_col = headers.get(EXPORT_HAWB_HEADER, 1)
    try:
        col_vals = _retry_api(ws.col_values, hawb_col,
                              label='export ensure_row read_col')
    except gspread.exceptions.APIError as e:
        logger.exception('export-row: col_values failed: %s', e)
        return None
    target_row_idx = 0
    for i, v in enumerate(col_vals, start=1):
        if i <= src.header_row:
            continue
        if (v or '').strip() == hawb.hawb_number:
            target_row_idx = i
            break

    if not target_row_idx:
        # Append. Заполняем сразу HAWB и transport_doc (mawb.awb_number).
        new_row = [''] * len(EXPORT_HEADERS_ORDER)
        new_row[EXPORT_HEADERS_ORDER.index(EXPORT_HAWB_HEADER)] = hawb.hawb_number
        if hawb.mawb_id and hawb.mawb:
            new_row[EXPORT_HEADERS_ORDER.index(EXPORT_TRANSPORT_DOC_HEADER)] = \
                hawb.mawb.awb_number or ''
        try:
            resp = _retry_api(ws.append_row, new_row,
                              value_input_option='RAW',
                              label='export append_row')
        except gspread.exceptions.APIError as e:
            logger.exception('export-row: append_row failed: %s', e)
            return None
        # gspread append_row возвращает {'updates': {'updatedRange': '...!A12:I12', ...}}
        target_row_idx = 0
        try:
            rng = (resp or {}).get('updates', {}).get('updatedRange', '')
            m = re.search(r'!([A-Z]+)(\d+)', rng)
            if m:
                target_row_idx = int(m.group(2))
        except Exception:
            pass
        if not target_row_idx:
            # Fallback — пересчитываем через col_values.
            target_row_idx = len(col_vals) + 1
        logger.info('export-row: appended HAWB %s at row %d',
                    hawb.hawb_number, target_row_idx)

    # Создаём ImportedSheetRow чтобы writeback функции находили эту строку.
    ImportedSheetRow.objects.update_or_create(
        source=src,
        source_row_index=target_row_idx,
        defaults={
            'hawb_number_raw':  hawb.hawb_number,
            'hawb_number_norm': hawb.hawb_number,
            'match_status':     'promoted',
            'promoted_hawb':    hawb,
            'matched_hawb':     hawb,
        },
    )
    return (src, target_row_idx)
...
def _kind_for_hawb(h: HouseWaybill) -> str:
    """Возвращает source_kind для HAWB: 'export' для EXPORT, иначе 'general'."""
    return 'export' if getattr(h, 'shipment_type', '') == 'EXPORT' else 'general'


def _write_hawb_date(hawb: HouseWaybill, value, header_name: str,
                     log_label: str,
                     export_header_name: str = '') -> bool:
    ...
    Для EXPORT-HAWB ищем строку в kind='export' вкладке, пишем в
    ...
    src_kind = _kind_for_hawb(hawb)
    if src_kind == 'export':
        if not export_header_name:
            return False  # колонка не определена для export — не пишем
        # Гарантируем что строка HAWB существует в export-вкладке.
        _ensure_export_row(hawb)

    date_str = _local_date_str(value)

    found = _find_row_for_kind(hawb, src_kind)
    if not found:
        logger.info('%s writeback skipped: HAWB %s has no row in general',
                    log_label, hawb.hawb_number)
        return False
    source, row_index = found
    effective_header = (export_header_name if src_kind == 'export'
                        else header_name)
    ...
    for attempt in range(len(backoff_steps) + 1):
        try:
            ws = open_worksheet(source)
            col = _ensure_named_column(ws, source.header_row, effective_header)
...
def write_release_date_for_hawb(hawb: HouseWaybill) -> bool:
    """Пишет HouseWaybill.release_date в Sheets.
...
def _batch_write_hawb_dates(hawbs: list, value_attr: str,
    ...
    source_kind — если None, авто-роутим по HAWB.shipment_type: EXPORT-HAWB
    ...
    # Авто-роутинг: разбиваем на две группы по shipment_type и обрабатываем
    # каждую отдельно. Это позволяет вызывать batch-функции из существующего
    # кода (inbox.py, outbox.py) без передачи source_kind — обе ветки
    # обработаются автоматически.
    #
    # Для export-группы пишем только если задан export_header_name — иначе
    # пропускаем (не все импортные CargoTrack-колонки нужны в экспортной
    # вкладке, у юзера там свои заголовки).
    if source_kind is None:
        exp = [h for h in hawbs if _kind_for_hawb(h) == 'export']
        imp = [h for h in hawbs if _kind_for_hawb(h) != 'export']
        total = 0
        if imp:
            total += _batch_write_hawb_dates(
                imp, value_attr, header_name, log_label,
                after_header=after_header, before_header=before_header,
                formatter=formatter, value_provider=value_provider,
                source_kind='general')
        if exp and export_header_name:
            total += _batch_write_hawb_dates(
                exp, value_attr, export_header_name, log_label,
                formatter=formatter, value_provider=value_provider,
                source_kind='export')
        return total
...
def batch_write_goods_count_for_hawbs(hawbs: list) -> int:
    ...
    return _batch_write_hawb_dates(
        ...
        formatter=_format_int,
        export_header_name=EXPORT_GOODS_COUNT_HEADER,
    )
...
def batch_write_customs_requests_for_hawbs(hawbs: list) -> int:
    ...
    return _batch_write_hawb_dates(
        ...
        export_header_name=EXPORT_CUSTOMS_REQUESTS_HEADER,
    )
...
def batch_write_customs_requests_count_for_hawbs(hawbs: list) -> int:
    ...
    return _batch_write_hawb_dates(
        ...
        export_header_name=EXPORT_CUSTOMS_REQUESTS_COUNT_HEADER,
    )
...
def batch_write_attempts_count_for_hawbs(hawbs: list) -> int:
    ...
    return _batch_write_hawb_dates(
        ...
        export_header_name=EXPORT_ATTEMPTS_COUNT_HEADER,
    )


def _transport_doc_for_hawb(hawb) -> str:
    """Транспортный документ HAWB (CDEK-XX-NNNN для экспорта, MAWB для импорта)."""
    return (hawb.mawb.awb_number if hawb.mawb_id and hawb.mawb else '') or ''


def _declaration_form_for_hawb(hawb) -> str:
    """Тип декларации (ПТДЭГ/ДТЭГ/ДТ) — пишем как есть."""
    return (hawb.declaration_form or '').strip()


def batch_write_transport_doc_for_hawbs(hawbs: list) -> int:
    """Batch writeback номера транспортного документа (mawb.awb_number) в
    «Номер транспортного документа» — только в export-вкладке.
    ...
    if not exp:
        return 0
    return _batch_write_hawb_dates(
        exp, '__transport_doc__',
        EXPORT_TRANSPORT_DOC_HEADER, 'transport_doc',
        formatter=lambda v: v or '',
        value_provider=_transport_doc_for_hawb,
        source_kind='export',
    )


def batch_write_declaration_form_for_hawbs(hawbs: list) -> int:
...
DEPRECATED_COLUMN_HEADERS = (
    ...
    # «дата подачи ДО1» — удалена 2026-05-26 (юзер: «убери, на её место поставь
    # дата регистрации ДО1»). svh_do1_sent_at per-HAWB больше не нужна,
    # достаточно scan_into_bond per-Cargo (= «дата регистрации ДО1»).
    'CargoTrack: дата подачи ДО1',
    # «дата ДО2» — удалена 2026-05-26 (юзер не использует).
    'CargoTrack: дата ДО2',
    # «номер партии» — удалена 2026-05-26 (юзер не использует, MAWB виден
    # в его колонке ТСД).
...
def batch_write_declarations_for_hawbs(hawbs: list, source_kind=None) -> int:
    ...
    Авто-роутинг по shipment_type: импортные HAWB пишутся в kind='general'
    ...
    if source_kind is None:
        ...
        total = 0
        if imp:
            total += batch_write_declarations_for_hawbs(imp, source_kind='general')
        if exp:
            total += batch_write_declarations_for_hawbs(exp, source_kind='export')
        return total

    # Для export — другое имя колонки (без префикса CargoTrack:).
    header_name = (EXPORT_DECLARATION_HEADER if source_kind == 'export'
                   else CARGOTRACK_COL_HEADER)
...
def write_declaration(hawb: HouseWaybill) -> bool:
    """Записывает hawb.customs_declaration_number в Sheets.

    Для импортной HAWB — в колонку «CargoTrack: ДТ» таблицы «Общее».
    Для экспортной HAWB — в «Регистрационный номер ДТ» вкладки export
    (при необходимости создаёт строку через _ensure_export_row).
    ...
    src_kind = _kind_for_hawb(hawb)
    if src_kind == 'export':
        # Гарантируем что строка HAWB в export-вкладке существует.
        _ensure_export_row(hawb)
    found = _find_row_for_kind(hawb, src_kind)
    if not found:
        logger.info('writeback skipped: HAWB %s has no row in %s sheet',
                    hawb.hawb_number, src_kind)
        return False
    source, row_index = found
    header_name = (EXPORT_DECLARATION_HEADER if src_kind == 'export'
                   else CARGOTRACK_COL_HEADER)
