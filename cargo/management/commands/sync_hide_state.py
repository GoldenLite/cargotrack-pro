...
class Command(BaseCommand):
    ...
    def _sync_tab(self, ss, ws):
        ...
        # ставится ТОЛЬКО при выпуске — старые ручные записи). Для DB и листовых
        # одинаково: рег.номер (last_decl) → скрываем. Строки БЕЗ рег.номера
        # (напр. 10265907412, last_decl='') сюда не попадают — остаются видимы.
        rows_to_hide = []
        rows_to_show = []
        idx_to_save = []
        for e in entries:
