...
class Command(BaseCommand):
    ...
    def _run(self, *args, **opts):
        ...
        for entry in qs.iterator(chunk_size=500):
            ...
            # Легаси-выпуск: рег.номер в CRM (кол.W → entry.last_decl) ставится
            # ТОЛЬКО при выпуске (старые ручные записи, до автоматизации). Если
            # статус ЭД не вычислился (нет сообщения о выпуске), но рег.номер
            # есть — считаем «Выпуск разрешен» (правило юзера 15.07.2026:
            # рег.номер = выпуск). Так строка получает статус И скрывается по
            # статус-ветке — для DB и листовых одинаково. compute даёт непустой
            # статус при реальном сообщении (напр. 'Присвоен номер') → override
            # НЕ срабатывает: не-выпущенные с реальным статусом не трогаем.
            if not new_status and entry.last_decl:
                new_status = 'Выпуск разрешен'
            ...
            # Legacy: рег.ДТ без статуса = выпущено (правило юзера: рег.номер
            # ставится ТОЛЬКО при выпуске). Обычно уже покрыто override'ом выше
            # (new_status='Выпуск разрешен' → скрытие по статус-ветке); оставлено
            # как страховка. НЕ исключаем db-строки: 10265907412 без рег.номера
            # (last_decl='') сюда не попадает и остаётся видимой.
